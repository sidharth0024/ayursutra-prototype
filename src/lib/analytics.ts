...
export const getAppointmentTrends = async (days: number = 30): Promise<AppointmentTrend[]> => {
  ...
  // Group by date
  const dateGroups: { [key: string]: { total: number; completed: number; cancelled: number } } = {}
...
export const getTherapyStats = async (): Promise<TherapyStats[]> => {
  const { data, error } = await supabase
    .from('appointments')
    .select('therapy:therapies(name), status')
...
export const getRecentActivity = async (limit: number = 10) => {
  const { data, error } = await supabase
    .from('appointments')
    .select('*, patient:patients(full_name), therapy:therapies(name)')
